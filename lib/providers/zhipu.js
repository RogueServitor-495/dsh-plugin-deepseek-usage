...
async function getJson(url, apiKey, opts = {}) {
  ...
  try {
    ...
    // Zhipu reports business errors as HTTP 200 + { code, msg } (e.g. code 1000
    // = auth failure, code 1100 = key invalid). Surface them as errors instead
    // of silently parsing empty data.
    if (json && typeof json === "object" && typeof json.code === "number" && json.code !== 0 && json.code !== 200) {
      const err = new Error(`Zhipu endpoint error: ${typeof json.msg === "string" ? json.msg : "code " + json.code}`);
      err.code = "ZHIPU_BIZ_ERROR";
...
export const zhipuAdapter = Object.freeze({
  ...
  async fetchBalance(apiKey, opts = {}) {
    // Account report is the primary signal: its failure (auth/network/biz)
    // propagates so callers see a real error instead of empty data.
    const account = parseAccountReport(await getJson(ZHIPU_HOST + ZHIPU_BALANCE_PATHS.accountReport, apiKey, opts));
    const packages = await getJson(ZHIPU_HOST + ZHIPU_BALANCE_PATHS.tokenAccounts, apiKey, opts)
      .then((v) => parseTokenAccounts(v))
      .catch(() => []); // token packages are supplementary
    const available = account.availableBalance !== void 0 ? Number(account.availableBalance) : NaN;
    return {
      currency: "CNY",
      isAvailable: Number.isFinite(available),
  ...
  async fetchUsage(apiKey, opts = {}) {
    // Quota is the primary signal; subscription (plan/version) is best-effort.
    const windows = buildQuotaWindows(extractLimits(await getJson(ZHIPU_HOST + ZHIPU_USAGE_PATHS.quota, apiKey, opts)));
    const sub = await getJson(ZHIPU_HOST + ZHIPU_USAGE_PATHS.subscription, apiKey, opts)
      .then((v) => parseSubscription(v))
      .catch(() => ({}));
    return {
      windows,
