...
/** @param {number} hourUtc8 - hour of day in Beijing time (0-23) */
function isInPeakWindow(hourUtc8) {
  return PEAK_WINDOWS.some((w) => hourUtc8 >= w.startHour && hourUtc8 < w.endHour);
}
...
export function isPeakHour(at) {
  ...
  return isInPeakWindow(hourUtc8);
}
...
export function aggregateEvents(events, start, end) {
  ...
  for (const event of events) {
    ...
    const bucket = isPeakHour(time) ? peak : offPeak;
