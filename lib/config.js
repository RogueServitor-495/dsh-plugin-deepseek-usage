...
/**
 * Quota-exhaustion auto-resume. `enabled: false` keeps the plugin purely
 * observational; enable it only when you want sessions interrupted by a
 * QUOTA failure to be woken automatically after the quota window resets.
 * @typedef {{
 *   enabled: boolean,
 *   maxAttempts: number,
...
/**
 * Official DeepSeek pricing (CNY per 1M tokens), per the peak/off-peak policy
 * effective 2026-08-17, taken from <https://api-docs.deepseek.com/zh-cn/quick_start/pricing/>.
 * Peak hours are Beijing 09:00-12:00 and 14:00-18:00; off-peak is half of peak.
 * Extend with more model ids as DeepSeek ships them.
 * @returns {ModelRatesMap}
 */
export function defaultModels() {
  return {
    "deepseek-v4-flash": {
      // Flash: off-peak 0.05 / 1.50 / 4.50, peak 0.10 / 3.00 / 9.00
      peak: { inputCacheMiss: 3.0, inputCacheHit: 0.1, output: 9.0 },
      offPeak: { inputCacheMiss: 1.5, inputCacheHit: 0.05, output: 4.5 }
    },
    "deepseek-v4-pro": {
      // Pro: off-peak 0.15 / 4.50 / 13.50, peak 0.30 / 9.00 / 27.00
...
/**
 * Supported providers for account/usage display.
 */
export const PROVIDER_CHOICES = Object.freeze(["deepseek", "zhipu"]);
...
export function normalizeConfig(raw) {
  ...
  const provider = typeof r.provider === "string" && PROVIDER_CHOICES.includes(r.provider) ? r.provider : fallback.provider;

  // resume: opt-in quota auto-resume; unknown fields fall back to defaults.
  const rawResume = r.resume && typeof r.resume === "object" ? r.resume : {};
  const resume = {
    enabled: typeof rawResume.enabled === "boolean" ? rawResume.enabled : fallback.resume.enabled,
    maxAttempts: typeof rawResume.maxAttempts === "number" && Number.isInteger(rawResume.maxAttempts) && rawResume.maxAttempts >= 1
      ? rawResume.maxAttempts
      : fallback.resume.maxAttempts,
    defaultRetryDelayMs: typeof rawResume.defaultRetryDelayMs === "number" && Number.isFinite(rawResume.defaultRetryDelayMs) && rawResume.defaultRetryDelayMs >= 10_000
      ? rawResume.defaultRetryDelayMs
      : fallback.resume.defaultRetryDelayMs
  };
