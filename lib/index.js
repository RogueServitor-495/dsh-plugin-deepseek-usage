...
import { installQuotaResume } from "./resume.js";
...
function apply(ctx, pluginConfig = {}) {
  ...
  ctx.effect(() => {
    ...
    // Quota-exhaustion auto-resume (opt-in via config.resume.enabled):
    // detects agent/request-error failures coded QUOTA, records a durable
    // pending-resume entry, and wakes the agent with a continuation message
    // after the provider's quota window resets. No agent-loop changes.
    disposers.push(installQuotaResume(ctx, {
      getConfig: () => config,
