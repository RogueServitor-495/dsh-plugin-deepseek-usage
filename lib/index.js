...
function makeHandler(ctx) {
  return async (req, res) => {
    ...
    try {
      ...
      const split = priceCnySplit(aggregate.usage, DEFAULT_PRICING);
      const now = Date.now();
      sendJson(res, 200, {
        ok: true,
        fetchedAt: now,
        ...
        cost: {
          cny: split.totalCny,
          usd: split.totalCny / DEFAULT_PRICING.usdCny,
          peakCny: split.peakCny,
          offPeakCny: split.offPeakCny
        },
        pricing: { ...DEFAULT_PRICING },
        period: {
          now: isPeakHour(now) ? "peak" : "offPeak",
          windows: PEAK_WINDOWS.map((w) => ({ ...w })),
