...
/**
 * Per-env credential resolution cache (env name → { value }); each entry
 * expires after 60s so a key change is picked up without a restart.
...
/**
 * Credential env names per provider, in resolution order (first configured
 * wins). Kimi accepts both the dedicated coding key and the generic one.
 * @type {Record<string, string[]>}
 */
const PROVIDER_KEY_ENVS = Object.freeze({
  deepseek: Object.freeze(["DEEPSEEK_API_KEY"]),
  zhipu: Object.freeze(["ZHIPU_API_KEY"]),
  kimi: Object.freeze(["KIMI_CODING_API_KEY", "KIMI_API_KEY"])
});

/**
 * Map a conversation's model/provider selection to a billing provider id.
 * Returns null when nothing matches (caller decides the fallback).
 * @param {string | null} providerHint - pi-ai provider id (e.g. kimi-coding)
 * @param {string | null} modelId - model id (e.g. k3-256k, glm-5.2)
 * @returns {string | null}
 */
function providerForModel(providerHint, modelId) {
  ...
  if (m.startsWith("kimi") || m.includes("moonshot") || /^k\d/.test(m)) return "kimi";
  return null;
}

/**
 * Resolve one provider's API key through the optional credentials seam,
 * trying each of its env names in order. Cached per env name for 60s.
 * @param {import("@deepseek-ai/cordis").Context} ctx
 * @param {string} providerId
 * @returns {Promise<string | null>}
 */
async function resolveApiKey(ctx, providerId) {
  const credentials = ctx.get("credentials");
  if (credentials === void 0) return null;
  for (const envName of PROVIDER_KEY_ENVS[providerId] ?? []) {
    const hit0 = keyCache.get(envName);
    if (hit0 !== void 0) {
      if (hit0.value !== null) return hit0.value;
      continue;
    }
    const hit = await credentials.resolve(envName);
    const value = asString(hit?.value);
    keyCache.set(envName, { value });
    setTimeout(() => {
      keyCache.delete(envName);
    }, 60_000).unref?.();
...
/**
 * Env names tried for a provider (for error messages).
 * @param {string} providerId
 * @returns {string}
 */
function providerKeyEnvsLabel(providerId) {
  return (PROVIDER_KEY_ENVS[providerId] ?? []).join(" or ");
}
...
/**
 * Last-seen model for sessions without live header events since this plugin
 * started (resolved once from the persisted log, then cached).
 * @type {Map<string, string | null>}
 */
const sessionModelStatic = new Map();

/**
 * Resolve the model currently in force for a session: the live request/header
 * map first (updated as the session streams), otherwise the last request/header
 * recorded in the persisted session log (cached per process).
 * @param {import("@deepseek-ai/cordis").Context} ctx
...
async function resolveSessionModel(ctx, sessionId) {
  ...
  const live = sessionModel.get(sessionId);
  if (live !== void 0) return live;
  if (sessionModelStatic.has(sessionId)) return sessionModelStatic.get(sessionId) ?? null;
  let model = null;
  try {
    const loaded = await loadSessionEvents(ctx, sessionId);
    const events = loaded?.events ?? [];
    for (let i = events.length - 1; i >= 0; i -= 1) {
      const event = events[i];
      if (isHeaderEvent(event)) {
        const m = modelOfHeader(event);
        if (m !== void 0) { model = m; break; }
      }
    }
  } catch {
    model = null; // unknown session / unreadable log: stay silent, fall back
  }
  sessionModelStatic.set(sessionId, model);
  return model;
}
...
function foldSessionEvents(events, start, end, windows) {
  ...
  for (const event of events) {
    ...
    // Header events update the in-force model regardless of the day window,
    // so usage recorded after a header that predates midnight still buckets
    // under the right model instead of "unknown".
    if (isHeaderEvent(event)) {
      const m = modelOfHeader(event);
      if (m !== void 0) model = m;
      continue;
 ...
 * @returns {{ peakCny: number, offPeakCny: number, totalCny: number, byModel: Record<string, { peakCny: number, offPeakCny: number, totalCny: number, estimated: boolean }> }}
 */
function priceDay(usage, byModel) {
  ...
  for (const model of modelIds) {
    ...
    byModelCost[model] = {
      peakCny: split.peakCny,
      offPeakCny: split.offPeakCny,
      totalCny: split.totalCny,
      // true when the model has no configured rates and was priced with the
      // fallback tier — the client marks such costs as rough estimates.
      estimated: !Object.prototype.hasOwnProperty.call(config.models, model)
    };
    peakCny += split.peakCny;
    offPeakCny += split.offPeakCny;
  }
...
function makeUsageHandler(ctx) {
  return async (req, res) => {
    ...
    try {
      // Current conversation context, supplied by the browser half: the
      // selected model/provider (from the model-selection service) and/or the
      // open session id (model resolved from live/logged request headers).
      const url = new URL(req.url ?? "/", "http://127.0.0.1");
      const sessionId = asString(url.searchParams.get("sessionId"));
      const providerHint = asString(url.searchParams.get("providerHint"));
      let model = asString(url.searchParams.get("model"));
      if (model === null && sessionId !== null) {
        model = await resolveSessionModel(ctx, sessionId);
      }

      // Pick the billing provider: fixed when configured, otherwise follow
      // the conversation's model. With no model context at all, auto mode
      // keeps the legacy default (deepseek) so the bar never goes blank.
      const providerAuto = config.provider === "auto";
      let providerId = config.provider;
      if (providerAuto) {
        providerId = providerForModel(providerHint, model)
          ?? (model === null && providerHint === null ? "deepseek" : "none");
      }
      const provider = PROVIDERS[providerId];

      const aggregate = await todayUsage(ctx);
      const priced = priceDay(aggregate.usage, aggregate.byModel);
      const now = Date.now();

      /** Shared payload core (usage/pricing are always available). */
      const core = {
        ok: true,
        fetchedAt: now,
        provider: provider?.id ?? "none",
        providerName: provider?.displayName ?? (model ?? "未识别厂商"),
        providerAuto,
        ...(model !== null ? { model } : {}),
        ...(sessionId !== null ? { sessionId } : {}),
        day: aggregate.day,
        usage: aggregate.usage,
        usageByModel: aggregate.byModel,
        cost: {
          cny: priced.totalCny,
      ...
      if (provider === void 0) {
        // Model maps to no known billing provider (e.g. a self-hosted
        // gateway): tokens + configured-rate estimate only, no account data.
        sendJson(res, 200, { ...core, currency: config.currency, balance: null, quotas: [] });
        return;
      }

      const key = await resolveApiKey(ctx, providerId);
      if (key === null) {
        // Degrade gracefully: usage/cost still render; the account segment
        // shows the missing-key hint instead of failing the whole request.
        sendJson(res, 200, {
          ...core,
          currency: config.currency,
          balance: null,
          ...
          accountError: {
            code: "NO_API_KEY",
            message:
              `${providerKeyEnvsLabel(providerId)} not configured. Store it through the web Models page (credentials service) or export it in the launching environment.`
          }
        });
        return;
      }

      // Balance and quota are independent surfaces: a failure of one must
      // not blank the other (or the usage core).
      const [balanceR, usageR] = await Promise.allSettled([
        provider.fetchBalance(key),
        provider.fetchUsage(key)
      ]);
      const balance = balanceR.status === "fulfilled" ? balanceR.value : null;
      const usageQ = usageR.status === "fulfilled" ? usageR.value : null;
      const accountError = balance === null && usageQ === null
        ? {
            code: "ACCOUNT_FETCH_FAILED",
            message: String(
              (balanceR.status === "rejected" && balanceR.reason && balanceR.reason.message)
              || (usageR.status === "rejected" && usageR.reason && usageR.reason.message)
      ...
      sendJson(res, 200, {
        ...core,
        currency: balance?.currency ?? config.currency,
        balance: balance === null ? null : {
          ...
          plan: usageQ?.plan,
          planVersion: usageQ?.planVersion,
          renewsAt: usageQ?.renewsAt
        },
        quotas: usageQ?.windows ?? [],
        ...(accountError !== void 0 ? { accountError } : {})
      });
    } catch (error) {
