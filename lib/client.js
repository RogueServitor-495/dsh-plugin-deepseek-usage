window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function mount(ctx) {
			...
			root.append(segTokens, el("span", "dsu-div"), segCost, el("span", "dsu-div"), segBalance, vPeriod, refreshBtn);
			...
			// The full scan behind /api/deepseek-usage is expensive (decompresses and
			// replays every session log, cached server-side for 5 minutes), so the
			// auto-refresh is kept at 5 minutes and tab-visibility auto-refresh is
			// intentionally omitted — only the initial load and manual clicks refresh.
