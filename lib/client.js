		...
		function mount(ctx) {
			...
			function openConfigPanel() {
				...
				function buildForm(cfg) {
					const peakTitle = el("div", "dsu-p-field");
					peakTitle.append(el("label", null, "高峰时段单价（元 / 百万 token）"));
					const peakRow = el("div", "dsu-p-row");
					peakRow.append(numField("缓存未命中", "peak.inputCacheMiss", cfg.peak.inputCacheMiss));
					peakRow.append(numField("缓存命中", "peak.inputCacheHit", cfg.peak.inputCacheHit));
					peakRow.append(numField("输出", "peak.output", cfg.peak.output));
					peakTitle.append(peakRow);
					panel.append(peakTitle);

					const offTitle = el("div", "dsu-p-field");
					offTitle.append(el("label", null, "空闲时段单价（元 / 百万 token）"));
					const offRow = el("div", "dsu-p-row");
					offRow.append(numField("缓存未命中", "offPeak.inputCacheMiss", cfg.offPeak.inputCacheMiss));
					offRow.append(numField("缓存命中", "offPeak.inputCacheHit", cfg.offPeak.inputCacheHit));
					offRow.append(numField("输出", "offPeak.output", cfg.offPeak.output));
					offTitle.append(offRow);
					panel.append(offTitle);
					...
					winField.append(el("label", null, "高峰时段（北京时间，24 小时制）"));
					const winRow = el("div", "dsu-p-row");
					winRow.append(numField("窗口1 开始", "peakWindows.0.startHour", cfg.peakWindows[0]?.startHour ?? 9, "1"));
					winRow.append(numField("窗口1 结束", "peakWindows.0.endHour", cfg.peakWindows[0]?.endHour ?? 12, "1"));
					winRow.append(numField("窗口2 开始", "peakWindows.1.startHour", cfg.peakWindows[1]?.startHour ?? 14, "1"));
					winRow.append(numField("窗口2 结束", "peakWindows.1.endHour", cfg.peakWindows[1]?.endHour ?? 18, "1"));
					winField.append(winRow, el("div", "dsu-p-hint", "区间为 [开始, 结束)，结束需大于开始。"));
